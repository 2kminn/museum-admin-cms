...
export function LocationsArtworksPage() {
  ...
  const [qrPreviewArtwork, setQrPreviewArtwork] = useState<ArtworkRecord | null>(null);
    ...
    <div className="space-y-5">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-[820px] w-full table-fixed text-left text-sm">
                      <thead className="bg-zinc-50 text-xs font-semibold text-zinc-600 dark:bg-zinc-950 dark:text-zinc-300">
                        <tr>
                          <th className="w-[156px] px-3 py-2">썸네일</th>
                          <th className="px-3 py-2">작품명 (KR)</th>
                          <th className="w-[140px] px-3 py-2">QR</th>
                          ...
                          filtered.map((item) => (
                            ...
                            >
                              <td className="px-3 py-2">
                                <div className="h-32 w-32 overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950">
                                  <ArtworkMediaPreview
                              ...
                              <td className="px-3 py-2">
                                ...
                                >
                                  {item.qrUrl ? (
                                    <button
                                      type="button"
                                      onClick={() => setQrPreviewArtwork(item)}
                                      className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-2.5 text-xs font-semibold text-zinc-900 shadow-sm hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:hover:bg-zinc-900"
                                    >
      ...
      {qrPreviewArtwork ? (
        <div
          ...
          aria-modal="true"
          aria-label="작품 QR 상세"
          onClick={() => setQrPreviewArtwork(null)}
        >
          <div
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                  {qrPreviewArtwork.localized.ko.title || "제목 없음"}
                </div>
                <div className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                  {qrPreviewArtwork.artist || "작가 미상"}
                </div>
              </div>
              <button
                type="button"
                onClick={() => setQrPreviewArtwork(null)}
                className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800"
                aria-label="QR 상세 닫기"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <ArtworkQrCard
              title={qrPreviewArtwork.localized.ko.title}
              artist={qrPreviewArtwork.artist}
              code={qrPreviewArtwork.code}
              qrUrl={qrPreviewArtwork.qrUrl}
            />
          </div>
        </div>
      ) : null}
